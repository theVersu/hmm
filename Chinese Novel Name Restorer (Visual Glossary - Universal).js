...
(function() {
    ...
    function updateRegexRules() {
        ...
        sortedKeys.forEach(badName => {
            const goodName = glossary[badName];
    ...
    function translateTextNode(node) {
        ...
        if (!isEngineActive || replacementRules.length === 0) return;
    ...
    function applyDataHashReplacements() {
        ...
        spans.forEach(span => {
            ...
            if (glossary[hash]) {
                span.textContent = glossary[hash];
            }
        });
    }
    ...
    function renderList() {
        ...
        filteredKeys.forEach(badName => {
            ...
            item.innerHTML = `
                <span class="gl-item-text" title="${badName} to ${glossary[badName]}">
                    <strong>${badName}</strong> <span class="gl-arrow">&rarr;</span> <strong>${glossary[badName]}</strong>
                </span>
                ...
                if (activeImportScope === 'CURRENT') {
                    ...
                    if (importedData && importedData._type === "FULL_NOVEL_GLOSSARY_BACKUP") {
                        ...
                        Object.keys(importedData.novels).forEach(key => {
                            ...
                            const merged = { ...existing, ...importedData.novels[key] };
                            GM_setValue(key, JSON.stringify(merged));
                        });
