...
(function() {
    ...
    // Determine target domain and target icon based on current host
    const isKemono = currentHost.includes('kemono');
    const targetDomain = isKemono ? 'pawchive.pw' : 'kemono.cr';
    
    // Select the opposite icon (if on Kemono, show Pawchive icon, and vice versa)
    ...
    // Construct target URL preserving path, search parameters, and hash
    ...
    // Create the icon inside the button using the Base64 data string
    ...
    // Observer fallback to ensure placement on dynamic single-page renders (e.g. Pawchive)
